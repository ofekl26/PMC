          ...
          <v-toolbar flat>
            <v-row align="center">
              ...
              <v-col align-self="center" cols="1" style="margin-top:21px">
                <v-text-field v-model="price" type="number" label="Up To" placeholder="ILS" shaped></v-text-field>
              </v-col>
            </v-row>
          </v-toolbar>
        </template>
...
export default {
  ...
  methods: {
          ...
          .then(querySnapshot => {
            querySnapshot.forEach(doc => {
              ...
              if (data.bookedUnit == currID) {
                console.log("test 1", data.bookedUnit, currID);
                res = true;
              }
    ...
    async curses(data) {
      ...
      console.log("test 2", data.unitID);
    },
